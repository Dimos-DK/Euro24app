...
### Load data - in PyCharm i changed to cached approach for performance reasons

# Define the directory containing the CSV files
directory = '/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/events'
...
# Find all files in the directory that end with 'shots-eurocopa-2024.csv'
...
# Load player info
players = pd.read_csv('/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/general_data/all_players-eurocopa-2024.csv')

# Load shots by player - file i created in another notebook
shots_by_player = pd.read_csv('/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/shots/euro24_shots_by_player.csv')
...
# Load a transformed events file i created
all_events = pd.read_csv('/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/general_data/euro_events_transformed.csv')

# Load the shots events with the xG value assigned with the model i created with the "Module11_Task1" notebook
xg_shot_events = pd.read_csv('/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/shots/euro24_xg_shots_splitted.csv')
...
# Create a English Team name version dictionary
team_names = {
    "GEORGIA" : "Georgia",
    "ROMANIA" : "Romania",
    "AUSTRIA" : "Austria",
...
# Rename Team Names to their English versions
events['Team Name'] = events['Team Name'].replace(team_names)

# Import the SICS glossary to rename columns to english
glossary = pd.read_csv('/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/general_data/glossary-euro-2024.csv')
...
# Divide each column (except 'Team' and 'matches') by the 'matches' column to create values per game and add the suffix '_avg' for distinction
...
# List of "negative" metrics that should be ranked in ascending order (inverted ranking)
...
# Rank the values in team_stats_avg
...
# Create another dataframe with inverted rankings
...
def display_scatter_plot(df, x_metric, y_metric):
    ...
    fig = px.scatter(
        ...
        labels={x_metric: x_metric_display_name, y_metric: y_metric_display_name}  # Use the descriptive names in the labels
    )
...
def display_bar_chart(df, metric):
    ...
    # Replace the metric name with the descriptive name dynamically
...
def create_team_passing_network(passing_network_data, team_name):
    ...
    average_locations = team_passing_network_data.groupby('Passer').agg({'Start X': ['mean'], 'Start Y': ['mean', 'count']})
    average_locations.columns = ['x', 'y', 'count']
    average_locations['y'] = 68-average_locations['y']

    # Calculate passes between players
    passes_between_players = team_passing_network_data.groupby(['Team Name','Passer', 'Recipient','Player From Name'])['Match ID'].count().reset_index(name='pass_count')
...
def plot_team_passing_network(events, players, team_name, match_id):
    ...
    passes['Accurate To Penalty Area'] = passes.apply(lambda x: 1 if x['outcome'] == '1' and x['To Penalty Area'] == 1 else 0, axis=1)

    # Drop Tags
    passes = passes.drop(columns=['Tags'])

    # Merge events with players to include 'Shirt Number' for Passer
    passes = passes.merge(players[['Player ID', 'Shirt Number']], left_on='Player From ID', right_on='Player ID', how='left')
    ...
    passes = passes.merge(players[['Player ID', 'Shirt Number']], left_on='Player To ID', right_on='Player ID', how='left')
    ...
    passing_network_data['Pass Length'] = np.sqrt((passing_network_data['End X'] - passing_network_data['Start X'])**2 + (passing_network_data['End Y'] - passing_network_data['Start Y'])**2)
    ...
    forward_passes = len(team_passing_network_data[team_passing_network_data['End X'] > team_passing_network_data['Start X']])
    forward_passes_pct = forward_passes / accurate_passes * 100
    passes_final_third = len(team_passing_network_data[(team_passing_network_data['Start X'] < 68) & (team_passing_network_data['End X'] > 68)])
    ...
    for _, row in team_passing_network[team_passing_network['pass_count'] >= pass_threshold].iterrows():
        color = cmap(norm(row['pass_count']))
        pitch.lines(
            row['x'], row['y'],
            row['x_end'], row['y_end'],
    ...
    # Add player nodes with edge colors based on count values
    nodes = pitch.scatter(
        team_passing_network['x'], team_passing_network['y'],
        s=25 * team_passing_network['count'].values,
        c='#E0FFFF',
        edgecolor=[edge_cmap(count_norm(val)) for val in team_passing_network['count'].values],  # Map count values to colors
        linewidth=4,
        ax=ax,
        alpha=0.5
    )
...
def plot_player_events(all_events, player, match_id):
    ...
    player_events['Pass Length'] = np.sqrt((player_events['End X'] - player_events['Start X'])**2 + (player_events['End Y'] - player_events['Start Y'])**2).round(2)
    ...
    # Define shapes and colors for different event descriptions
    event_styles = {
    ...
    for event, style in event_styles.items():
        ...
        pitch.scatter(event_data['Start X'], 68-event_data['Start Y'],
                      s=style['size'], color=style['color'], marker=style['marker'], edgecolors='black', ax=ax, label=event)
    ...
    event_counts = {event: len(player_events[player_events['Event Description'] == event]) for event in event_styles.keys()}
    ...
    # Add title
    ax.annotate(f"Spotlight on {player_events['Player From Name'].iloc[0]} - {player_events['Game'].iloc[0]}", xy=(0.5, 0.99), xytext=(0, 0),fontsize=15, xycoords='axes fraction', textcoords='offset points',
                    va='top', ha='center', color='#FF4500', fontweight='bold')
...
def plot_player_shots(shot_events, player_name, match_id=None):
    ...
    handles = []
    for event, style in event_styles.items():
        handles.append(plt.Line2D([0], [0], marker=style['marker'], color='w', label=f"{event} ({len(player_data[player_data['Outcome'] == event])})",
                                  markerfacecolor=style['color'], markersize=10, markeredgewidth=1, markeredgecolor='red'))
...
def plot_team_shots(shot_events, team_name, match_id=None):
    ...
    # Add the custom legend in the middle
...
# Load the logos
sdc_logo_path = '/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/Negra verTR.png'
euro24_logo_path = '/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/Euro24_Team_Files/euro-2024-logo.png'
sics_logo_path = '/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/Euro24_Team_Files/SICS_Logo.jpeg'
dk_logo_path = '/content/drive/Othercomputers/My Mac/Downloads/Euro 2024 project/DK_Analytics_logo copy.webp'
...
tabs = st.tabs(["Passing Networks", "Team Head 2 Head", "Team Comparison", "Top 10", "Rankings Card", "Player Spotlight", "Shot Maps"])
...
with tabs[0]:
    ...
    with col1:
        team_name = st.selectbox('Select Team:', options=sorted(all_events['Team Name'].unique()), key='pn_team_name', help='Select Team')
...
with tabs[1]:
    ...
    col1, col2 = st.columns(2)
    with col1:
        team1 = st.selectbox('Select TeamA:', sorted(team_stats_full['Team'].unique()), key='radar_team1', help='Select the first team')

    with col2:
        team2 = st.selectbox('Select TeamB (optional):', [''] + sorted(list(team_stats_full['Team'].unique())), key='radar_team2', help='Select the second team (optional)')

    # Dropdown for metrics selection
    metrics_option = st.selectbox('Select Category:', ['Passing', 'Finishing', 'OnBall'], help='Select the metrics category')
...
with tabs[4]:
    ...
    col1, col2 = st.columns(2)
    with col1:
        team_for_card = st.selectbox('Select Team for Rankings Card:', sorted(team_stats_full['Team'].unique()), key='team_for_card', help='Select the team for the rankings card')
...
# Hover effect with tooltip and "i" help button
    ...
    # Select the team
    with col1:
        team_name = st.selectbox('Select Team:', sorted(all_events['Team Name'].dropna().unique()), key='ps_team_name', help='Select the Team of interest')
    ...
    with col3:
        filtered_players = all_events[(all_events['Team Name'] == team_name) & (all_events['Game'] == game)]['Player From Name'].dropna().unique()
        player_name = st.selectbox('Select Player:', sorted(filtered_players), key='ps_player_name', help='Select the Player of interest')
...
with tabs[6]:
    ...
    with col2:
        ...
        if team_name:
            filtered_games = ['All'] + sorted(xg_shot_events[xg_shot_events['Team Name'] == team_name]['Game'].unique().tolist())
